package com.github.rulegin.actors.service.cluster.discovery;

import com.github.rulegin.dao.util.MiscUtils;
import com.github.rulegin.server.gen.discovery.ServerInstanceProtos;
...
public class ZkDiscoveryService implements DiscoveryService, PathChildrenCacheListener, ApplicationListener<ApplicationReadyEvent> {
    ...
    public void childEvent(CuratorFramework curatorFramework, PathChildrenCacheEvent pathChildrenCacheEvent) throws Exception {
        ...
        if (data == null) {
            log.debug("Ignoring {} due to empty child dataSource", pathChildrenCacheEvent);
            return;
        } else if (data.getData() == null) {
            log.debug("Ignoring {} due to empty child's dataSource", pathChildrenCacheEvent);
            return;
        ...
        try {
            instance = new ServerInstance(ServerInstanceProtos.ServerInfo.parseFrom(data.getData()));
        } catch (IOException e) {
